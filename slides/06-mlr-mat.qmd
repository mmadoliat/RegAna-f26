...
$$\hat{\bf y} = {\bf X} {\bf b} = \color{red}{{\bf X} ({\bf X}' {\bf X}) ^{-1} {\bf X'}} \color{black}{{\bf y} =} \color{red}{\bf H} \color{black}{\bf y}$$
- The residuals are $e_i = y_i - \hat{y}_i$, and the residual vector is
$${\bf e} = {\bf y} - \hat{\bf y} ={ \bf y} - {\bf X} {\bf b} = {\bf y} -{\bf H} {\bf y} = ({\bf I} - {\bf H}) {\bf y}$$

. . .

::: alert
- Both $\mathbf{H}$ and $\mathbf{I} - \mathbf{H}$ are **symmetric and idempotent**; hence, **orthogonal projection matrices**.
- $\mathbf{H}$ projects $\mathbf{y}$ onto the column space of $\mathbf{X}$: $\qquad\qquad\qquad\hat{\mathbf{y}} = \mathbf{H}\mathbf{y}\in \operatorname{Col}(\mathbf{X}).$
- $\mathbf{I} - \mathbf{H}$ projects $\mathbf{y}$ onto the orthogonal complement of the $\operatorname{Col}(\mathbf{X})$: $\ \ \  \mathbf{e}=(\mathbf{I}-\mathbf{H})\mathbf{y}\in\operatorname{Col}(\mathbf{X})^\perp.$
:::

$$\boxed{\mathbf{y}=\hat{\mathbf{y}}+\mathbf{e},\qquad\hat{\mathbf{y}} \perp \mathbf{e}}$$
