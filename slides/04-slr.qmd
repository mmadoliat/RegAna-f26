...
abline(v = mean(mpg$displ), col="grey", lty=2); abline(h = mean(mpg$hwy), col="grey", lty=2)
...
```
[Regression Illustration](http://tinyurl.com/MLABshiny/Regression/ "Shiny App")
...
draw_right_plot <- function(stage = 0) {
  par(mar = c(0, 0, 0, 0))
  plot(y_s[1:40] ~ x[1:40], xlab = "", ylab = "", yaxt = "n", xaxt = "n", pch = 19,
       col = "green3", xlim = range(x), ylim = range(c(y_s, y_s_1)))
  abline(lm(y_s[1:40] ~ x[1:40]), col = "blue", lwd = 2)
  abline(v = x[40], lty = 2, col = "red")
  segments(x0 = -3, y0 = min(y_s)-0.5, x1 = x[40], y1 = min(y_s)-0.5, col = 2, 
         lwd = 3)
...
y_outlier <- c(y[1:5], y[6]+200, y[7:11])
plot(y_outlier ~ x, pch = 19, col = "black", 
     xlab = "", ylab = "", xaxt = "n", yaxt = "n", ylim=c(y[11],y[6]+200))
...
## Considerations in the Use of Regression: Influential Points
- **Outlier**: An observation that is considerably different from the rest of the data (unusual in $y$ direction)
